...
class Dogecoin extends Controller
{
    ...
    public function HitungSMA($table)
    {
        ...
        // Prepare the SQL query to get the low, high, and volume values from the dogecoin table in groups of 5
    ...
    public function Threshold($table)
    {
        ...
        // Prepare the SQL query to get the monthly averages of low, high, and volume from the dogecoin table
    ...
    // treshold check based on threshold table per month and iterate to chech per day in dogecoin table
    public function bayes($table)
    {
        ...
        foreach ($thresholds as $threshold) {
            ...
            // Get the dogecoin data for the month
...
public function f1Score()
{
    ...
    public function predict()
    {
        ...
        // count all data in table dogecoin and iterate
    ...
    //dogecoin
    public function importdogecoin(Request $request)
    {
        $datei = $request->date;
        // validate date
        ...
        if ($file && $file->isValid()) {
            ...
            $table = 'dogecoin';
            DB::table('dogecoin')->where('id', '<>', 'admin')->delete();
            foreach ($data as $row) {
        ...
        // get data as array from table dogecoin and column low and column id
        $low_data = DB::table($table)->select('low')->get();
        $low_trend = DB::table('SMA')->select('sma_low')->get();

        // get data as array from table dogecoin and column volume and column id
        ...
        // select high and low and volume from table dogecoin based on date
    ...
    {
        // minim date in table dogecoin
        $minDate = DB::table('dogecoin')->min('date');
        $maxDate = DB::table('dogecoin')->max('date');
        return view('menu.dogecoin', compact('minDate', 'maxDate'));
    }
}
