...
class Tron extends Controller
{
    ...
    public function HitungSMA($table)
    {
        ...
        // Prepare the SQL query to get the low, high, and volume values from the tron table in groups of 5
    ...
    public function Threshold($table)
    {
        ...
        // Prepare the SQL query to get the monthly averages of low, high, and volume from the tron table
    ...
    // treshold check based on threshold table per month and iterate to chech per day in tron table
    public function bayes($table)
    {
        ...
        foreach ($thresholds as $threshold) {
            ...
            // Get the tron data for the month
...
public function f1Score()
{
    ...
    public function predict()
    {
        ...
        // count all data in table tron and iterate
    ...
    //tron
    public function importtron(Request $request)
    {
        $datei = $request->date;
        // validate date
        ...
        if ($file && $file->isValid()) {
            ...
            $table = 'tron';
            DB::table('tron')->where('id', '<>', 'admin')->delete();
            foreach ($data as $row) {
        ...
        // get data as array from table tron and column low and column id
        $low_data = DB::table($table)->select('low')->get();
        $low_trend = DB::table('SMA')->select('sma_low')->get();

        // get data as array from table tron and column volume and column id
        ...
        // select high and low and volume from table tron based on date
    ...
    {
        // minim date in table tron
        $minDate = DB::table('tron')->min('date');
        $maxDate = DB::table('tron')->max('date');
        return view('menu.tron', compact('minDate', 'maxDate'));
    }
}
