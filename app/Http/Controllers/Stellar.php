...
class Stellar extends Controller
{
    ...
    public function HitungSMA($table)
    {
        ...
        // Prepare the SQL query to get the low, high, and volume values from the stellar table in groups of 5
    ...
    public function Threshold($table)
    {
        ...
        // Prepare the SQL query to get the monthly averages of low, high, and volume from the stellar table
    ...
    // treshold check based on threshold table per month and iterate to chech per day in stellar table
    public function bayes($table)
    {
        ...
        foreach ($thresholds as $threshold) {
            ...
            // Get the stellar data for the month
...
public function f1Score()
{
    ...
    public function predict()
    {
        ...
        // count all data in table stellar and iterate
    ...
    //stellar
    public function importstellar(Request $request)
    {
        $datei = $request->date;
        // validate date
        ...
        if ($file && $file->isValid()) {
            ...
            $table = 'stellar';
            DB::table('stellar')->where('id', '<>', 'admin')->delete();
            foreach ($data as $row) {
        ...
        // get data as array from table stellar and column low and column id
        $low_data = DB::table($table)->select('low')->get();
        $low_trend = DB::table('SMA')->select('sma_low')->get();

        // get data as array from table stellar and column volume and column id
        ...
        // select high and low and volume from table stellar based on date
    ...
    {
        // minim date in table stellar
        $minDate = DB::table('stellar')->min('date');
        $maxDate = DB::table('stellar')->max('date');
        return view('menu.stellar', compact('minDate', 'maxDate'));
    }
}
