...
class Solana extends Controller
{
        ...
        public function HitungSMA($table)
        {
            ...
            // Prepare the SQL query to get the low, high, and volume values from the solana table in groups of 5
        ...
        public function Threshold($table)
        {
            ...
            // Prepare the SQL query to get the monthly averages of low, high, and volume from the solana table
        ...
        // treshold check based on threshold table per month and iterate to chech per day in solana table
        public function bayes($table)
        {
            ...
            foreach ($thresholds as $threshold) {
                ...
                // Get the solana data for the month
    ...
    public function f1Score()
    {
        ...
        public function predict()
        {
            ...
            // count all data in table solana and iterate
        ...
        //solana
        public function importsolana(Request $request)
        {
            $datei = $request->date;
            // validate date
            ...
            if ($file && $file->isValid()) {
                ...
                $table = 'solana';
                DB::table('solana')->where('id', '<>', 'admin')->delete();
                foreach ($data as $row) {
            ...
            // get data as array from table solana and column low and column id
            $low_data = DB::table($table)->select('low')->get();
            $low_trend = DB::table('SMA')->select('sma_low')->get();
    
            // get data as array from table solana and column volume and column id
            ...
            // select high and low and volume from table solana based on date
        ...
        {
            // minim date in table solana
            $minDate = DB::table('solana')->min('date');
            $maxDate = DB::table('solana')->max('date');
            return view('menu.solana', compact('minDate', 'maxDate'));
        }
    }
