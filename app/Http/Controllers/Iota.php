...
class Iota extends Controller
{
    ...
    public function HitungSMA($table)
    {
        ...
        // Prepare the SQL query to get the low, high, and volume values from the iota table in groups of 5
    ...
    public function Threshold($table)
    {
        ...
        // Prepare the SQL query to get the monthly averages of low, high, and volume from the iota table
    ...
    // treshold check based on threshold table per month and iterate to chech per day in iota table
    public function bayes($table)
    {
        ...
        foreach ($thresholds as $threshold) {
            ...
            // Get the iota data for the month
...
public function f1Score()
{
    ...
    public function predict()
    {
        ...
        // count all data in table iota and iterate
    ...
    //iota
    public function importiota(Request $request)
    {
        $datei = $request->date;
        // validate date
        ...
        if ($file && $file->isValid()) {
            ...
            $table = 'iota';
            DB::table('iota')->where('id', '<>', 'admin')->delete();
            foreach ($data as $row) {
        ...
        // get data as array from table iota and column low and column id
        $low_data = DB::table($table)->select('low')->get();
        $low_trend = DB::table('SMA')->select('sma_low')->get();

        // get data as array from table iota and column volume and column id
        ...
        // select high and low and volume from table iota based on date
    ...
    {
        // minim date in table iota
        $minDate = DB::table('iota')->min('date');
        $maxDate = DB::table('iota')->max('date');
        return view('menu.iota', compact('minDate', 'maxDate'));
    }
}
